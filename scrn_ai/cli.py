...
@click.option("--output", "-o", type=click.Path(), required=True, help="Output .h5ad file path.")
@click.option("--min-genes", type=int, default=200, help="Minimum genes per cell (default: 200).")
@click.option("--min-cells", type=int, default=3, help="Minimum cells per gene (default: 3).")
@click.option(
    "--max-genes", type=int, default=None, help="Maximum genes per cell (filter high outliers)."
)
@click.option(
    "--max-mito-pct",
    type=float,
...
@main.command()
@click.option(
    "--input", "-i", type=click.Path(exists=True), required=True, help="Input .h5ad file."
)
...
@click.option(
    ...
    "-m",
    default="seurat",
    type=click.Choice(["seurat", "jmp", "log1p", "scran", "sctransform"], case_sensitive=False),
...
@click.option(
    ...
    type=click.Choice(
        ["LogNormalize", "SCTransform", "TMM", "RLE", "UpperQuartile"], case_sensitive=False
    ),
...
@click.option(
    "--output", "-o", type=click.Path(), required=True, help="Output image file (.png, .pdf, etc.)."
)
@click.option(
    "--color-by", "-c", default="leiden", help="Observation key to color by (default: leiden)."
)
@click.option(
    "--n-neighbors", type=int, default=15, help="Number of neighbors for UMAP (default: 15)."
)
@click.option(
    "--min-dist", type=float, default=0.1, help="Minimum distance for UMAP (default: 0.1)."
)
@click.option(
    "--cell-types",
    type=click.Path(exists=True),
...
@click.option(
    "--output", "-o", type=click.Path(), required=True, help="Output directory or .h5ad file."
)
@click.option(
    "--method",
    "-m",
...
@click.option(
    "--root-cell", type=str, default=None, help="Root cell ID for pseudotime calculation."
)
...
@click.option(
    "--timing",
    type=click.Choice(["pre_analysis", "post_analysis", "both"], case_sensitive=False),
...
@click.option("--species", default="human", help="Species (human, mouse, etc., default: human).")
@click.option("--tissue", default=None, help="Tissue type if known (e.g., brain, blood).")
@click.option(
    "--cluster-key",
    default="leiden",
...
@click.option("--species", "-s", required=True, help="Species name tag stored in AnnData.uns.")
@click.option(
    "--method", "-m", default="dpt", type=click.Choice(["dpt", "dtflow"], case_sensitive=False)
)
...
@click.option("--root_cell", type=str, default=None, help="Cell ID to use as VIA root.")
@click.option("--outfile", "-o", required=True, type=click.Path())
...
@main.command()
@click.option("--infiles", "-i", multiple=True, required=True, type=click.Path(exists=True))
@click.option("--outfile", "-o", type=click.Path(), required=True)
