...
def identify_variable_genes(
    adata: AnnData, n_top_genes: int = 2000, flavor: str = "seurat_v3", subset: bool = False
) -> list[str]:
    """
    ...
    sc.pp.highly_variable_genes(adata, n_top_genes=n_top_genes, flavor=flavor, subset=subset)
...
def filter_marker_genes(
    ...
    for gene in marker_genes:
        ...
        if custom_exclude and any(pattern.upper() in gene_upper for pattern in custom_exclude):
            continue
...
def get_top_markers_per_cluster(
    ...
    **kwargs
    ...
    all_markers = find_cluster_markers(
        adata, cluster_key=cluster_key, n_genes=n_genes_initial, **kwargs
    )
    ...
    if filter_genes:
        ...
        filtered_markers = {
            cluster_id: genes[:n_markers] for cluster_id, genes in all_markers.items()
        }
...
def validate_marker_genes(
    marker_genes: Union[list[str], dict[str, list[str]]], adata: AnnData
    ...
    if isinstance(marker_genes, dict):
        ...
        for cluster_id, genes in marker_genes.items():
            ...
            if len(valid_genes) < len(genes):
                ...
                logger.warning(
                    "Cluster %s — %d marker genes not found in dataset", cluster_id, len(missing)
                )
...
def get_marker_expression_summary(
    adata: AnnData, marker_genes: list[str], cluster_key: str = "leiden"
) -> pd.DataFrame:
    """
