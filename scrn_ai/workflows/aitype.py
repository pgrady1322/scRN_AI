...
from ..utils.openai_client import CellTypePrediction, OpenAIClient
...
def run(
    ...
    if marker_genes:
        ...
        cluster_markers = get_top_markers_per_cluster(
            adata, cluster_key=cluster_key, n_markers=n_markers, filter_genes=True
        )

    # Filter to clusters we're processing
    cluster_markers = {
        str(c): markers
        for c, markers in cluster_markers.items()
    ...
    try:
        client = OpenAIClient(api_key=api_key, model=model, temperature=0.3)
    ...
    predictions = client.batch_predict_cell_types(
        clusters=cluster_markers, species=species, tissue=tissue, context=context_info
    )
    ...
    logger.info(
        "Output: %s_annotations.csv, %s_confidence_scores.csv, %s_marker_genes.csv, %s_reasoning.txt",
        timing,
        timing,
        timing,
        timing,
...
def _save_results(
    ...
    for cluster_id, markers in cluster_markers.items():
        marker_data.append(
            {"cluster": cluster_id, "marker_genes": ", ".join(markers), "n_markers": len(markers)}
        )
